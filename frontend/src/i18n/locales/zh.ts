  ...
  admin: {
    ...
    groups: {
      ...
      form: {
        ...
        expiresAt: '过期时间',
        expiresAtHint: '留空表示永不过期。到期后分组会自动暂停。',
        exclusiveLabel: '专属分组',
    ...
    subscriptions: {
      ...
      validityHint: '按天数分配订阅；如填写最终到期时间，则忽略天数',
      expiresAtHint: '也可以直接选择最终到期时间，需晚于当前时间',
      adjustingFor: '为以下用户调整订阅',
      currentExpiration: '当前到期时间',
      adjustDaysPlaceholder: '正数延长，负数缩短',
      adjustHint: '输入正数延长订阅，负数缩短订阅；也可直接填写最终到期时间',
      adjustExpiresAtHint: '直接设置订阅最终到期时间，需晚于当前时间',
      assign: '分配',
      assigning: '分配中...',
      adjust: '调整',
      adjusting: '调整中...',
      ...
      validityDaysRequired: '请输入有效的天数（至少1天）或最终到期时间',
      expiresAtRequired: '请选择最终到期时间',
      expiresAtMustBeFuture: '最终到期时间必须晚于当前时间',
      adjustInputRequired: '请输入调整天数或最终到期时间',
      adjustInputExclusive: '调整天数和最终到期时间只能填写一个',
